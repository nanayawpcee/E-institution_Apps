...
/// Profile tab: identity card, availability and appearance switches, then the
/// account and support menus.
...
class _UserProfileScreenState extends ConsumerState<UserProfileScreen> {
      ...
      child: ListView(
        ...
        children: [
          ...
          _IdentityCard(
            ...
            imagePath: tutor?.userImage ?? '',
            onEditImage: _showImageOptions,
          ),
          const SizedBox(height: 14),
          _SwitchRow(
            icon: Icons.how_to_reg_outlined,
            label: 'Open to new students',
            value: tutor?.available ?? true,
            onChanged: (v) =>
                ref.read(authProvider.notifier).updateAvailability(v),
          ),
  ...
  Future<void> _editContact() async {
    final controller =
        TextEditingController(text: ref.read(authProvider)?.contact ?? '');
...
/// Card-framed labelled switch, as used for availability and dark mode.
class _SwitchRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(icon, size: 19, color: t.text),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              label,
              style: TLText.body(t.text).copyWith(fontWeight: FontWeight.w600),
            ),
          ),
