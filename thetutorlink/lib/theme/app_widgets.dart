...
class TLNavItem {
  ...
  /// Supplies the glyph instead of [icon]/[activeIcon] — used where a tab
  /// needs its own artwork or a badge, such as the chat tab.
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            item.iconBuilder?.call(context, selected, color) ??
                Icon(
...
/// Metric tile: tinted glyph well, figure, label. Sized for a phone grid.
class TLStatTile extends StatelessWidget {
  const TLStatTile({
    Key? key,
    required this.icon,
    required this.value,
    ...
    return TLCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: TLText.figure(t.text).copyWith(fontSize: 22)),
...
/// Circular progress dial with its percentage in the middle.
class TLProgressRing extends StatelessWidget {
  const TLProgressRing({
    Key? key,
    required this.value,
    this.size = 44,
    this.color = TLTokens.primary,
      ...
      child: Stack(
        ...
        children: [
          ...
          Text(
            ...
            style: TLText.tag(t.text).copyWith(fontSize: size * 0.26),
          ),
        ],
      ),
    );
